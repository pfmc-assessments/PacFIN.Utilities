...
##############################################################################
#
# plotCleanedAges plot age data after cleanAges
#
##############################################################################
...
plotCleanedAges = function(Pdata) {
  ...
  # Number of ages by state and year

  barplot(xtabs(Pdata$FREQ ~ Pdata$state + Pdata$fishyr), col=rainbow(3),
          legend.text = T, xlab = "Year", ylab = "Age Samples",
          main=paste("Species", SPID))

  # Ages
  boxplot(Pdata$age ~ Pdata$fishyr, ylab="Ages",
          main=paste("Species", SPID))

} # End function plotCleanedAges
...
KFJgetComps = function( Pdata, strat=NULL, Comps="AAL" ) {
  ...
  # KFJ: significantly reduce the amount of code and merging
  # end results is the same
  # Use towstrat to fix bug in Ntows, where bug was carried forward
  # in write comps as well.

  Cstrat = c(strat, usualSuspects)
  towstrat <- c("SEX", Cstrat[!Cstrat %in% c("lengthcm", "age")])
  ...
  test <- aggregate(Pdata[, c("Final_Sample_Size", "FREQ")],
                    Pdata[, c("SEX", Cstrat)], sum, na.rm = TRUE)

  test <- merge(test, aggregate(Pdata$SAMPLE_NO, Pdata[, towstrat], lenique),
                by = towstrat, all = TRUE)

  temp <- reshape(test, timevar = "SEX", idvar = Cstrat, direction = "wide")

  colnames(temp) <- c(Cstrat, "female", "fsamps", "ftows", "male", "msamps",
                              "mtows", "unsexed", "usamps", "utows")

  # KFJ: make sure all columns are returned

  # Andi -- NOT SURE THIS IS RIGHT

  temp <- temp[, c(Cstrat, "male", "msamps", "mtows", "female", "fsamps",
                           "ftows", "unsexed", "usamps", "utows")]

  invisible(temp)

} # End function KFJ_getComps
